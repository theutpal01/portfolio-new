...
const Label = ({className, text} : {className: string, text: string}) => {
	const delay = Math.random() * 0.5 + 0.1; // Random delay between 0.1 and 0.6 seconds
	return (
		<motion.span
...
const Footer = () => {
  return (
	<footer className='flex flex-col gap-4 h-fit overflow-x-hidden'>
		<Divider />
		<motion.div 
		...
		<motion.div 
			...
			whileInView={{ opacity: 1, y: 0 }} 
			id="trigger-end" className='relative -z-10 flex justify-center w-fit self-center'>
			<h2	className='text-primary select-none text-center overflow-hidden font-nova-round text-[7rem] sm:text-[11rem] md:text-[13rem] lg:text-[16rem] xl:text-[19rem] leading-none'>
				Thanks
			</h2>
