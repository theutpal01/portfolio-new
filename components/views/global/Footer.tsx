...
const Label = ({className, text} : {className: string, text: string}) => {
	  return (
	<span className={`${className} bg-background hidden md:block border-2 border-primary rounded-full px-5 py-3 font-semibold text-xs md:text-sm lg:text-base xl:text-lg text-primary`}>
		{text}
	</span>
  )
}
...
const Footer = () => {
  return (
	<footer className='flex flex-col gap-4'>
		<Divider />
		<div className='flex flex-col md:flex-row gap-5 gap-y-3 font-aldrich text-primary text-sm md:text-xl md:items-center z-0'>
		...
		<div id="trigger-end" className='relative -z-10 flex justify-center w-fit self-center'>
			<h2 className='text-primary select-none text-center font-nova-round text-[7rem] sm:text-[11rem] md:text-[13rem] lg:text-[17rem] xl:text-[20rem] leading-none'>
				Thanks
			</h2>
